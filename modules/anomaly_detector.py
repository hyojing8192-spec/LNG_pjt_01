...
def detect_smp_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    """
    SMP 이상구간 탐지.
    ...
    mask_zero = df["smp"] <= SMP_ZERO_THRESHOLD
    mask_high = df["smp"] >= SMP_HIGH_THRESHOLD

    results = []
    for _, row in df[mask_zero | mask_high].iterrows():
        atype = "SMP 제로" if row["smp"] <= SMP_ZERO_THRESHOLD else "SMP 과대"
        results.append({
...
def build_smp_chart(df: pd.DataFrame, anomalies: pd.DataFrame) -> go.Figure:
    """
    ...
    # 임계선
    fig.add_hline(
        y=SMP_HIGH_THRESHOLD, line_dash="dash",
        line_color="#FF8C00", opacity=0.6,
        annotation_text=f"과대 기준 {SMP_HIGH_THRESHOLD}원",
    )
    fig.add_hline(
        y=0, line_dash="dot",
