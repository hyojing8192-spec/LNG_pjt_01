...
FEATURE_COLS = MODEL_FEATURES + ["hour_sin", "hour_cos", "month_sin", "month_cos"]
...
def time_series_train_test_split(
    ...
    """
    시계열 순서 유지. 앞쪽 (1−test_fraction) = train, 뒤쪽 = test (셔플 없음).
    테스트 구간은 학습·교차검증·하이퍼파라미터 튜닝에 사용하지 않는 최종 평가용.
    """
...
def _choose_cv_split(n: int):
    """긴 구간은 시간 순서 CV, 짧은 구간은 셔플 KFold (n_splits < n)."""
    if n < 5:
...
def _tune_and_train(
    ...
    """XGBoost 학습. R² < MIN_R2 시 간단한 재튜닝 적용."""
    ...
    r2_mean = float(np.nanmean(scores))
...
def train_all_models(df: pd.DataFrame) -> dict:
    ...
    for mode in MODES:
        ...
        if mode == "low2gi" and len(subset_tr) < 50:
            border = df_train[
                (df_train["lng_gen"] >= 380_000) & (df_train["lng_gen"] < 560_000)
            ].dropna(subset=FEATURE_COLS)
            subset_tr = pd.concat([subset_tr, border]).drop_duplicates()
            print(f"[INFO] low2gi train 보강: {len(subset_tr)}행 (경계구간, train만)")
