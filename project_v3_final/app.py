...
with st.sidebar:
    ...
    if df_tmp is not None and not df_tmp.empty:
        ...
        _auto_rate = auto_exchange_rate_prev_day_mean(df_tmp, target_date)
    else:
        lng_heat = 9.1
        _auto_rate = 1447.0
    ...
    exchange_rate = st.number_input(
        ...
        value=round(_auto_rate, 0),
        step=1.0,
        help=f"전일 관측 평균 {_auto_rate:,.0f}원. 직접 수정 가능.",
    )
    st.caption(f"전일 자동: {_auto_rate:,.0f} 원/$")
...
with tab_econ:
    ...
    if day_df.empty:
        ...
        # ── 최적 운전모드 Gantt 스타일 (단일 트레이스, 성능 개선) ──
        mode_color_map = {v: MODE_COLORS.get(k, "#999") for k, v in MODE_LABELS.items()}
        bar_colors = [mode_color_map.get(row["최적모드"], "#999") for _, row in econ_table.iterrows()]
        bar_labels = econ_table["최적모드"].tolist()
        ...
        fig_mode = go.Figure(go.Bar(
            ...
            hovertemplate=[
                f"<b>{row['시간']}</b><br>최적모드: {row['최적모드']}<extra></extra>"
        ...
        for mode_key, label in MODE_LABELS.items():
            fig_mode.add_trace(go.Bar(
                ...
                marker_color=MODE_COLORS.get(mode_key, "#999"),
            ))
...
with tab_anomaly:
    ...
    if anom_df.empty:
        ...
        # SMP 임계값 슬라이더 (사용자 조정 가능)
        col_smp1, col_smp2 = st.columns(2)
        with col_smp1:
            smp_high_thr = st.slider(
                "SMP 과대 기준 (원/kWh)", min_value=100, max_value=300, value=170, step=5,
                help="이 값 이상이면 SMP 과대로 탐지"
            )
        with col_smp2:
            smp_zero_thr = st.slider(
                "SMP 제로 기준 (원/kWh)", min_value=0, max_value=20, value=0, step=1,
                help="이 값 이하이면 SMP 제로로 탐지"
            )

        smp_anomalies = detect_smp_anomalies(
            anom_df,
            zero_threshold=float(smp_zero_thr),
            high_threshold=float(smp_high_thr),
        )
        ...
        with c3:
            st.metric(f"SMP 과대 구간 (≥{smp_high_thr})", f"{cnt_high}시간")

        # SMP 차트
        fig_smp = build_smp_chart(anom_df, smp_anomalies, high_threshold=float(smp_high_thr))
        ...
        # 해당 컬럼이 없으면 실시간 계산
        if selected_col not in anom_df.columns:
            st.info(f"'{selected_col}' 컬럼이 데이터에 없어 SMP 기준으로 대체 표시합니다.")
            # SMP 시계열로 급변 탐지 (대체)
            _tmp = anom_df[["datetime","smp"]].rename(columns={"smp": selected_col})
            change_df = detect_econ_change(_tmp, selected_col, threshold)
            plot_df   = _tmp
        else:
        ...
        fig_change = build_econ_change_chart(plot_df, change_df, selected_col)
