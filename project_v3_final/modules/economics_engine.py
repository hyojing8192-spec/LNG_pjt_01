...
def calc_economics(
    ...
    경제성(억원/월)    = 경제성차이 × 발전량(kW) × 월운전시간(h) / 1e8

    [월 경제성 계산]
    엑셀 V/W/X열: (SMP - 대체단가) * 발전량(kW) * 52(h/월) * 환율 / 1e8
    단, 경제성차이가 원/kWh 단위이므로 환율 곱셈은 불필요 (이미 원화)
    → 경제성(억원) = 경제성차이(원/kWh) × 발전량(kW) × 52(h) / 1e8

    BEP 정보도 함께 반환 (가동 가능 여부 판단).
    """
    from config import HOURS_PER_MONTH

    econ_diff = smp - replace_cost          # 원/kWh

    capacity = {
        "1gi":    287_500.0,
        "low2gi": 410_000.0,                # 저부하 대표 발전량
        "2gi":    575_000.0,
    }.get(mode, lng_gen_kw if lng_gen_kw > 0 else 287_500.0)

    # 발전량이 실측으로 들어왔으면 실측 우선
    gen_kw = lng_gen_kw if (lng_gen_kw and lng_gen_kw > 0) else capacity

    # 경제성(억원) = 경제성차이(원/kWh) × 발전량(kW) × 52시간 / 1억
    econ_bil = econ_diff * gen_kw * HOURS_PER_MONTH / 1e8

    # viable: BEP > LNG가 (연료비 회수 가능)
    viable = (bep is not None and bep > lng_price) or econ_diff > 0
...
def build_hourly_table(
    ...
    for hour in range(24):
        ...
        for mode in MODES:
            ...
            econ  = calc_economics(mode, smp, rc, exchange_rate, lng_gen_h, eff, lng_heat, lng_price, bep)
